...
SPISettings settingsSD(FreqSD3, MSBFIRST, SPI_MODE0); 
...
void Bitmap::SDfromfile(const char* filename)
{
  SdFat SD(3);
  const uint8_t SD_CS = PA15;
  File archivo;
  SD.begin(SD_CS, SD_SCK_MHZ(FreqSPI3));
...
//const uint16_t GDClass::TAM_BUFFER_SD=8192;
//const uint16_t GDClass::TAM_BUFFER_FT=1024;
const uint16_t GDClass::TAM_BUFFER_SD=128;
const uint16_t GDClass::TAM_BUFFER_FT=64;
byte GDClass::bufT[TAM_BUFFER_SD];
byte GDClass::FTbufT[TAM_BUFFER_FT];
//FT81XMANIA TEAM (RndMnkIII)
...
// ******************************************************************************************
//    REGISTOS CORRECTOS PARA FT813  5"    26 FEBRERO DE 2018 Julio C. González (TFTLCDCyg)
// ******************************************************************************************
...
// ******************************************************************************************
//    REGISTOS CORRECTOS PARA FT813 4.3"    26 FEBRERO DE 2018 Julio C. González (TFTLCDCyg)
// ******************************************************************************************
...
#ifdef __F103VET6__
 #define i2c_address 0x50    //EEPROM shield o en F103VET6
#endif
...
#if defined(ARDUINO) && !defined(ESP32) && !defined(__DUE__)
// ********************* FT81XMANIA by @lightcalamar **************************************
    if ((EEPROM.read(0) != 0x7c)) {
      self_calibrate();
      // for (int i = 0; i < 24; i++) Serial.println(GDTR.rd(REG_TOUCH_TRANSFORM_A + i), HEX);
...
void GDClass::self_calibrate(void) {
  ...
  GD.Begin(BITMAPS);
  ColorRGB(255,255,0);  GD.Vertex2ii((GD.w / 2)-50, (GD.h / 2)-130, 0, 0);
  ColorRGB(255,255,255);  

  //cmd_text(240, 100, 30, OPT_CENTERX, "please tap on the dot");

  cmd_text(GD.w / 2, GD.h / 2, 30, OPT_CENTERX, "Toca el punto para calibrar touch");

  cmd_bgcolor(0xffffff);
...
void GDClass::printNfloat(int16_t x, int16_t y, float f, int16_t Presc, byte font) {
 ...
 //Parte entera
int fadd=font;
if(font<=15){fadd=80;}  //FT81X Uso de romfont 32, 33 o 34
    ...
    while( offset < tamArchivo)
    {
        ...
        while (offsetFT < m) 
        {
              ...
              if (progress)
                ...
                memcpy (FTbufT, bufT+offsetFT, wbn );
#else
                GD.copyram((bufT+offsetFT), wbn);
#endif              
              offsetFT+=n;
              bytesDisponibles-=n;
              GDTR.stop();
...
//FT81XMANIA TEAM (TFTLCDCyg)
  ...
  //cmd_text(GD.w/2, GD.h/2, 29, OPT_CENTER, message);
  cmd_dlstart();
  ...
  Clear();
   ...
   cmd_text(GD.w / 2, (GD.h / 2) - (GD.h / 20), 30, OPT_CENTERX, "ERROR");
   cmd_text(GD.w / 2, (GD.h / 2) + (GD.h / 20), 30, OPT_CENTERX, "Revisar comandos:");
   cmd_text(GD.w / 2, (GD.h / 2) + 3*(GD.h / 20), 28, OPT_CENTERX, "- Falta GD.Swap");
   cmd_text(GD.w / 2, (GD.h / 2) + 4*(GD.h / 20), 28, OPT_CENTERX, "- Revisar nombre de archivo png o jpg");
   cmd_text(GD.w / 2, (GD.h / 2) + 5*(GD.h / 20), 28, OPT_CENTERX, "- Grabar archivo png o jpg en la tarjeta microSD");
   cmd_text(GD.w / 2, (GD.h / 2) + 6*(GD.h / 20), 28, OPT_CENTERX, "- Revisar sintaxis de instrucciones");

  //cmd_text(GD.w/2, (GD.h/2)-29, 30, OPT_CENTER, "ERROR");
...
//RndMnkIII: m\E9todo safeloadSdFat para carga de assets desde lector SDIO tarjetas Teensy 3.5/3.6
